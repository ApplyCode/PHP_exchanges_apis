...
class HuobiAPI
{
    ...
    function __construct($key, $secret, $url = 'https://api.huobi.pro', $version = '0', $sslverify = true)
    {
        $this->key = $key ?? '';
        $this->secret = $secret ?? '';
        $this->url = $url;
    ...
    // public function QueryPrivate($httpMethod = 'POST', $method, array $params = [])
    public function QueryPrivate($httpMethod = 'POST', $method, $bodyData)
    {
        print_r($bodyData);
...
try {
    ...
    // ];
    // // $jsonEncodedOrdersData = json_encode($orderData);
    // print_r($jsonEncodedOrdersData);
    // echo "json date\n";
