// Debugging output
            if ($price >= $upperThreshold) {
                ...
                echo "Before Increment - Positive Fluctuations for $pct: " . $fluctuations[$pct]['positive'] . "\n"; // Debugging line
                $fluctuations[$strPct]['positive']++;
                echo "After Increment - Positive Fluctuations for $pct: " . $fluctuations[$strPct]['positive'] . "\n"; // Debugging line
...
function getKrakenRecentTrades($pair, $since, $count = null) {
    $url = "https://api.kraken.com/0/public/Trades?pair=" . $pair . "&since=" . $since;
    if ($count !== null) {
        $url .= "&count=" . $count;
    }
...
function analyzeTrades($trades, $price, $pcts)
{
    ...
    foreach ($trades as $trade) {
        ...
        foreach ($pcts as $pct) {
            $strPct = (string)$pct;
            $upperThreshold = $price * (1 + $pct);
            $lowerThreshold = $price * (1 - $pct);
            if ($price >= $upperThreshold) {
                $fluctuations[$strPct]['positive']++;
...
try {
    ...
    $price = 51000; // Base price for comparison
    ...
    $response = getKrakenRecentTrades($market, $since);
    print_r($response);

    // Analyze the trades
    if (!isset($response['error']) || count($response['error']) == 0) {
        $trades = $response['result'][$market];
        list($fluctuations, $averageVolumes) = analyzeTrades($trades, $price, $pcts);
