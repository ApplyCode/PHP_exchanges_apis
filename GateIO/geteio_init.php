...
class GateAPI
{
    ...
    {
        return time() * 1000; // Gate.io generally uses milliseconds since epoch
    }
    ...
    private function signature($httpMethod, $requestPath, $request, $timestamp)
    {
        ...
        $fmt = "%s\n%s\n%s\n%s\n%s";
        $signature_string = sprintf($fmt, $method, $requestPath, $queryString, $hashedPayload, $timestamp);
    ...
    public function queryPrivate($httpMethod = 'POST', $method, array $request = [])
    {
        ...
        $signature = $this->signature($httpMethod, $path, $request, $timestamp);
        $bodyData = null;
        $headers = [
        ...
        switch (strtoupper($httpMethod)) {
            case 'GET':
                $url = $this->url . 'api/' . $this->version . '/' . $method . '?' . $params;
                break;
            case 'POST':
                $bodyData = $params;
                $url = $this->url . $path;
...
try {
    ...
    $batchTradingFee = $gateApi->queryPrivate('GET', 'spot/batch_fee', ['currency_pair' => 'BTC_USDT']);
    ...
    $createBatch = $gateApi->queryPrivate('POST', 'spot/batch_orders', ["text" => "t-123456", "currency_pair" => "ETH_BTC", "type" => "limit", "account" => "spot", "side" => "buy", "iceberg" => "0", "amount" => "1", "price" => "5.00032", "time_in_force" => "gtc", "auto_borrow" => false, "stp_act" => "cn"]);
    ...
    $createOrder = $gateApi->queryPrivate('POST', 'spot/orders', ["text" => "t-123456", "currency_pair" => "ETH_BTC", "type" => "limit", "account" => "spot", "side" => "buy", "iceberg" => "0", "amount" => "1", "price" => "5.00032", "time_in_force" => "gtc", "auto_borrow" => false, "stp_act" => "cn"]);
    ...
    $order_id = 1223;
    $amendOrder = $gateApi->queryPrivate('PATCH', 'spot/orders/' . $order_id, ['currency_pair' => 'BTC_USDT']);
    echo "Get a single order\n";
    print_r($amendOrder);
    ...
    $batchModification = $gateApi->queryPrivate('POST', 'spot/amend_batch_orders', ["order_id" => "121212", "currency_pair" => "BTC_USDT", "account" => "spot", "amount" => "1", "amend_text" => "test"]);
