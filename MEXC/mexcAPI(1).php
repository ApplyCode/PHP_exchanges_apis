...
class MEXCAPI
{
    ...
    private function executeRequest($url, $bodyData = null, $headers = [])
    {
    ...
    public function QueryPrivate($httpMethod = 'POST', $method, array $request = [])
    {
        ...
        if (isset($request['batchOrders'])) {
            $batchOrderData = urlencode($request['batchOrders']);
            $queryString = "batchOrders={$batchOrderData}&timestamp={$timeStamp}";
        }
        $postdata = http_build_query($request, '', '&');
        $sign = hash_hmac('sha256', $postdata, $this->secret);
        $bodyData = null;
        $url = $this->url . 'api/' . $this->version . '/' . $method;
        if ($method == 'batchOrders') {
            $bodyData = '';
            ...
            switch (strtoupper($httpMethod)) {
                ...
                case 'DEL':
                    ...
                    break;
                case 'POST':
                    // For POST, the signature is part of the body
        ...
        return $this->executeRequest($url, $bodyData, $headers);
...
try {
    ...
    $cancelOrder = $mexc->QueryPrivate("DEL", "order", ["symbol" => "MXUSDT", "orderId" => "1"]);
    echo "Cancel Order\n";
    print_r($cancelOrder);
